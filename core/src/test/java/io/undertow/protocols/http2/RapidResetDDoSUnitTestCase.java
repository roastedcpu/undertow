...
public class RapidResetDDoSUnitTestCase {
        ...
        final PathHandler path = new PathHandler()
                .addExactPath(MESSAGE, new HttpHandler() {
                    ...
                    public void handleRequest(HttpServerExchange exchange) throws Exception {
                        RapidResetDDoSUnitTestCase.sendMessage(exchange);
                    }
                })
                .addExactPath(POST, new HttpHandler() {
                    ...
                    public void handleRequest(HttpServerExchange exchange) throws Exception {
                        exchange.getRequestReceiver().receiveFullString(new Receiver.FullStringCallback() {
                            @Override
                            public void handle(HttpServerExchange exchange1, String message) {
    ...
    public void assertDoSRstFramesHandled(final int totalNumberOfRequests, final int rstStreamLimit, final boolean errorExpected, final URI serverAddress) throws Exception {
        final List<ClientResponse> responses = new CopyOnWriteArrayList<>();
        final CountDownLatch latch = new CountDownLatch(totalNumberOfRequests);

        ServiceLoader<ClientProvider> providers = doPrivileged(new PrivilegedAction<ServiceLoader<ClientProvider>>() {
            @Override
            public ServiceLoader<ClientProvider> run() {
                return ServiceLoader.load(ClientProvider.class, RapidResetDDoSUnitTestCase.class.getClassLoader());
            }
        });
        ...
        final ClientCallback<ClientConnection> listener = new ClientCallback<ClientConnection>() {
        ...
        final OptionMap tlsOptions = OptionMap.builder()
        ...
        ChannelListener<StreamConnection> openListener = new ChannelListener<StreamConnection>() {
            ...
            public void handleEvent(StreamConnection connection) {
                ALPNClientSelector.runAlpn((SslConnection) connection,
                        new ChannelListener<SslConnection>() {
                            @Override
                            public void handleEvent(SslConnection connection1) {
                                UndertowLogger.ROOT_LOGGER.alpnConnectionFailed(connection1);
                                IoUtils.safeClose(connection1);
                            }
                        },
                        listener,
                        alpnProtocol(listener, serverAddress.getHost(), DefaultServer.getBufferPool(), tlsOptions)
                );
            }
        };
        ...
        ssl.openSslConnection(worker, new InetSocketAddress(serverAddress.getHost(), serverAddress.getPort()), openListener, tlsOptions).addNotifier(
                new IoFuture.Notifier<StreamConnection, Object>() {
                    @Override
    ...
    public static ALPNClientSelector.ALPNProtocol alpnProtocol(final ClientCallback<ClientConnection> listener, final String defaultHost, final ByteBufferPool bufferPool, final OptionMap options) {
        return new ALPNClientSelector.ALPNProtocol(
                new ChannelListener<SslConnection>() {
                    @Override
                    public void handleEvent(SslConnection connection) {
                        listener.completed(createHttp2Channel(connection, bufferPool, options, defaultHost));
                    }
                }, HTTP2);
    }
