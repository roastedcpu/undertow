...
/**
 * ClientConnection implementation that mimics the DDoS rapid reset attack. See UNDERTOW-2323.
 *
 * @author Stuart Douglas
 * @author Flavia Rainone
 */
public class DoSHttp2ClientConnection extends Http2ClientConnection implements ClientConnection {
    ...
    public DoSHttp2ClientConnection(Http2Channel http2Channel, boolean initialUpgradeRequest, String defaultHost, ClientStatistics clientStatistics, boolean secure) {
        ...
        ChannelListener<Http2Channel> closeTask = new ChannelListener<Http2Channel>() {
            @Override
            public void handleEvent(Http2Channel channel) {
                ChannelListeners.invokeChannelListener(DoSHttp2ClientConnection.this, closeSetter.get());
                for (ChannelListener<ClientConnection> listener : closeListeners) {
                    listener.handleEvent(DoSHttp2ClientConnection.this);
                }
    ...
    public void sendRequest(ClientRequest request, ClientCallback<ClientExchange> clientCallback) {
        ...
        //UNDERTOW-2323 send a rst stream right away
        http2Channel.sendRstStream(sinkChannel.getStreamId(), Http2Channel.ERROR_CANCEL);
    ...
    private void handleError(Throwable t) {
        ...
        IoUtils.safeClose(DoSHttp2ClientConnection.this);
        for (Map.Entry<Integer, Http2ClientExchange> entry : currentExchanges.entrySet()) {
            try {
        ...
        private class ContinueReceiveListener implements ChannelListener<AbstractHttp2StreamSourceChannel> {
            ...
            public void handleEvent(AbstractHttp2StreamSourceChannel sourceChannel) {
                ...
                try {
                    ...
                    int statusCode = Integer.parseInt(Objects.requireNonNull(channel.getHeaders().getFirst(STATUS)));
        ...
        public void handleEvent(Http2Channel channel) {
            try {
                ...
                if (result instanceof Http2StreamSourceChannel) {
                    ...
                    int statusCode = Integer.parseInt(Objects.requireNonNull(streamSourceChannel.getHeaders().getFirst(STATUS)));
                    ...
                    if(statusCode < 200) {
                        ...
                        if(statusCode == 100) {
                            ...
                            streamSourceChannel.getReadSetter().set(new Http2ReceiveListener.ContinueReceiveListener(http2Channel));
                    ...
                    if(request == null) {
                        ...
                        Http2ClientExchange newExchange = new Http2ClientExchange(
                                DoSHttp2ClientConnection.this, null, cr);
                    ...
                    goAwayStreamSourceChannel = (Http2GoAwayStreamSourceChannel) result;
                    close();
                } else if(result != null) {
                    Channels.drain(result, Long.MAX_VALUE);
                }
    ...
    /**
     * Returns the last Http2GoAwayStreamSourceChannel received, if any.
     * @return the last go away frame received
     */
    public Http2GoAwayStreamSourceChannel getGoAwayStreamSourceChannel() {
        return goAwayStreamSourceChannel;
    }
}
